...
public interface EmployeeCsatSummaryRepo extends CrudRepository<EmployeeCsatSummary, Integer> {
	...
	String totalCsat = "select avg(csat) as avg from employee_csat_summary where employee_csat_summary.employee_email=?1";
	@Query(value = totalCsat, nativeQuery = true)
	public double getAvgCsatScore(String empEmail);
	
	//for a particular account
	@Query(value ="select avg(csat) as avg from employee_csat_summary where employee_email=?1 and account_id=?2", nativeQuery = true)
	public double getAvgScoreOfAccount(String email,int accId);

	
	String Distinct_Employee_Query = "select distinct employee_email from employee_csat_summary where account_id=?1";
